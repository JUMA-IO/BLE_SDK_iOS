...
//    JMLog(@"%s, %@, %@, %@, %@", __func__, peripheral, characteristic, error, [NSThread currentThread]);
    ...
    if ([characteristic.UUID isEqual: [JumaDeviceConstant notifyCharacteristicUUID]])
    {
        if (!error)
        {
            ...
            {
//                _isRegistered = YES;
                // 该设备已经注册过, JumaManager 通知 delegate 连接成功
//                if (_isRegistered)
//                {
...
//                else
//                {
//                    const unsigned char bytes[] = { JumaDataType80, 0x01, 0x00 };
...
//    JMLog(@"%s, %@, %@, %@", __func__, peripheral, characteristic, error);
    ...
    {
        // 可以连接该设备并且已经连接成功
//        if (JumaDeviceStateConnected == _state)
//        {
...
//        }
//        // 在尚未知晓能不能连接该设备的情况下, 只有进行注册检查才会向 peripheral 写入数据
//        else if (JumaDeviceStateConnecting == _state)
//        {
//            if (error)
//            {
//                self.canNotEstablishConnectionError = error;
...
//        }
//        // 在尚未知晓能不能连接该设备的情况下, 只有进行注册检查才会从 peripheral 获取数据
//        else if (JumaDeviceStateConnecting == _state)
//        {
//            if (!error)
//            {
////                [self signIn];
//                [self checkWetherRegistered:characteristic.value];
//            }
//            else
//            {
//                self.canNotEstablishConnectionError = error;
